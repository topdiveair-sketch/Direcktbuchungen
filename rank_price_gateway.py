...
from wachauetappe_gateway import app
...
function iso(d){return d.toISOString().slice(0,10)}const today=new Date();const tomorrow=new Date(today);tomorrow.setDate(today.getDate()+1);arrival.value=iso(today);departure.value=iso(tomorrow);
