...
def partner_host_id():
    ...
    th=sha(auth[7:].strip())
    with db() as c:
        row=c.execute("SELECT host_id,expires_at FROM wachauetappe_partner_sessions WHERE token_hash=?",(th,)).fetchone()
...
def partner_login():
    p=request.get_json(silent=True) or {};host_id=str(p.get('hostId') or '').strip();email=str(p.get('email') or '').strip().lower();code=str(p.get('accessCode') or '')
...
def partner_save_range():
    ...
    with db() as c:
        while d<=t:
            c.execute("""INSERT INTO wachauetappe_partner_availability(host_id,stay_date,status,rooms_free,price,breakfast_mode,breakfast_price,luggage_available,updated_at)
            VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(host_id,stay_date) DO UPDATE SET status=excluded.status,rooms_free=excluded.rooms_free,price=excluded.price,breakfast_mode=excluded.breakfast_mode,breakfast_price=excluded.breakfast_price,luggage_available=excluded.luggage_available,updated_at=excluded.updated_at""",(host_id,d.isoformat(),status,rooms,price,breakfast,breakfast_price,luggage,now()));n+=1;d+=timedelta(days=1)
...
def partner_provision():
    ...
    p=request.get_json(silent=True) or {};host_id=str(p.get('hostId') or '').strip();name=str(p.get('name') or '').strip();location=str(p.get('location') or '').strip();email=str(p.get('email') or '').strip().lower();code=str(p.get('accessCode') or '')
    if not host_id or not name or not location or '@' not in email or len(code)<8:return cors(jsonify({'error':'required_fields'})),422
