...
def growth_action():
    ...
    if kind == "publish":
        ...
        public_url = request.host_url.rstrip("/") + f"/growth/publications/{publication_id}"
        return jsonify({"ok": True, "published": True, "publicUrl": public_url}), 201
...
def growth_publication_page(publication_id: int):
    ...
    cta = (
        ...
        if row["brand"] == "zuhause_am_bach" and booking_url
        else ""
    )
