...
function saveTripDashboard(refs){if(!activeBooking)return;const data={savedAt:new Date().toISOString(),references:refs,guests:Number($('bookingGuests').value||1),guestName:$('guestName').value.trim(),guestEmail:$('guestEmail').value.trim(),items:activeBooking.items.map(x=>({hostId:x.hostId,name:x.name,location:x.location,date:x.date,price:x.price??null})),luggage:!!currentTrip?.luggage,startDate:currentTrip?.startDate||activeBooking.items[0]?.date||null};localStorage.setItem('wachauetappeTrip',JSON.stringify(data));window.dispatchEvent(new CustomEvent('wachauetappe-trip-saved',{detail:data}))}
...
async function submitBooking(e){
  ...
  try{
    ...
    saveTripDashboard(refs);setBookingState(`✓ Reiseanfrage übertragen. Referenzen: ${refs.join(', ')}. Die Gastgeber bestätigen nun ihre Nächte. Deine Reise wurde außerdem unter „Meine Reise“ auf diesem Gerät gespeichert.`);submit.textContent='Anfrage gesendet ✓';
