...
track('planner_view',{route:$('route').value});closeBooking();const tomorrow=new Date();tomorrow.setDate(tomorrow.getDate()+1);$('startDate').value=iso(tomorrow);$('stayDate').value=iso(tomorrow);loadData().catch(err=>{console.error(err);alert('WachauEtappe konnte die Routendaten nicht laden.')});