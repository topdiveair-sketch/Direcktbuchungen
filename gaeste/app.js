...
function hostMeta(h){const id=h?.hostId||h?.id;return hosts.find(x=>String(x.id)===String(id))||hosts.find(x=>eq(x.name,h?.name))||null}
function featureChips(h){const m=hostMeta(h),items=(h?.features||m?.features||[]).slice(0,4);return items.map(x=>`<span class="chip">${esc(x)}</span>`).join('')}
function track(event,details={}){try{fetch(`${API_BASE}/api/wachauetappe/funnel`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event,...details}),keepalive:true}).catch(()=>{})}catch(e){}}
function routeStrip(days){const stops=[days[0]?.from,...days.map(d=>d.to)].filter(Boolean);return `<div class="route-strip"><div class="route-strip-line"></div>${stops.map((p,i)=>`<div class="route-stop"><i>${i+1}</i><span>${esc(p)}</span></div>`).join('')}</div>`}
function saveTripDashboard(refs,tripKey,tripReference){if(!activeBooking)return;const data={savedAt:new Date().toISOString(),references:refs,tripKey,tripReference,overallStatus:'pending',guests:Number($('bookingGuests').value||1),guestName:$('guestName').value.trim(),guestEmail:$('guestEmail').value.trim(),items:activeBooking.items.map((x,i)=>({hostId:x.hostId,name:x.name,location:x.location,date:x.date,price:x.price??null,reference:refs[i]||'',status:'requested'})),luggage:!!currentTrip?.luggage,startDate:currentTrip?.startDate||activeBooking.items[0]?.date||null};localStorage.setItem('wachauetappeTrip',JSON.stringify(data));window.dispatchEvent(new CustomEvent('wachauetappe-trip-saved',{detail:data}))}
...
async function renderTrip(days,startDate){
  track('trip_planned',{route:$('route').value,nights:Math.max(0,days.length-1),luggage:$('luggage').checked});
  ...
  $('tripResult').innerHTML=`<div class="trip-summary"><strong>${days.length} Reisetage</strong><span>${total.toFixed(1)} km gesamt</span><span>${esc(startDate)} bis ${esc(addDays(startDate,days.length-1))}</span><span>${luggage?'Gepäckservice gewünscht':'ohne Gepäckservice'}</span></div>${routeStrip(days)}<div class="trip-days" id="tripDays"></div><div id="tripBookingAction" class="panel" style="margin-top:18px" hidden></div>`;
  const wrap=$('tripDays');
...
function renderTripHosts(dayIndex){
  ...
  if(!d.hosts.length){box.innerHTML=d.online?'<div class="empty"><strong>Keine freie Partnerunterkunft gefunden.</strong><br>Für diese Nacht ist aktuell kein freies Kontingent gemeldet.<div class="empty-actions"><button class="btn secondary" type="button" data-we="inquiry">Alternative anfragen</button></div></div>':'<div class="empty"><strong>Online-Verfügbarkeit derzeit nicht erreichbar.</strong><br>Es wird kein Betrieb als frei ausgegeben, solange die Live-Prüfung fehlt.<div class="empty-actions"><button class="btn secondary" type="button" data-we="inquiry">Persönlich anfragen</button></div></div>';return}
  ...
  box.innerHTML=note+`<div class="host-grid" style="grid-template-columns:repeat(auto-fit,minmax(250px,1fr));margin-top:0">${d.hosts.map((h,j)=>{const id=h.hostId||h.id,selected=d.selected&&String(d.selected.hostId)===String(id),m=hostMeta(h);return `<article class="host-card host-card-rich" style="outline:${selected?'3px solid #173D32':'none'}"><div class="host-card-top"><span class="eyebrow">${d.online?'LIVE FREI':'PARTNER'}</span>${m?.accommodation_type?`<small>${esc(m.accommodation_type)}</small>`:''}</div><h3>${esc(h.name)}</h3><div class="location">${esc(h.location||d.to)}</div><div class="host-price">${h.price!=null?`<strong>${money(h.price)}</strong><span>für diese Anfrage</span>`:'<strong>Preis auf Bestätigung</strong><span>direkt vom Gastgeber</span>'}</div><div class="chips"><span class="chip">1 Nacht</span><span class="chip">Direktzahlung</span>${featureChips(h)}</div>${m?.cancellation?.free_until_days_before_arrival?`<p class="host-note">Kostenfrei stornierbar bis ${m.cancellation.free_until_days_before_arrival} Tage vor Anreise laut Gastgeberkondition.</p>`:''}<button class="btn ${selected?'secondary':'primary'}" type="button" data-select-trip-host="${dayIndex}:${j}">${selected?'Ausgewählt ✓':'Gastgeber auswählen'}</button></article>`}).join('')}</div>`;
  box.querySelectorAll('[data-select-trip-host]').forEach(btn=>btn.addEventListener('click',()=>{const [di,hi]=btn.dataset.selectTripHost.split(':').map(Number);const h=currentTrip.days[di].hosts[hi];currentTrip.days[di].selected={hostId:h.hostId||h.id,name:h.name,location:h.location||currentTrip.days[di].to,date:currentTrip.days[di].date,price:h.price??null};track('host_selected',{route:$('route').value,nights:Math.max(0,currentTrip.days.length-1),luggage:currentTrip.luggage});renderTripHosts(di);updateTripAction()}));
}
...
function renderHosts(results,date,guests,online=true){const box=$('stayResult');if(!results.length){box.innerHTML='<div class="empty"><strong>Keine freie Partnerunterkunft gefunden.</strong><br>Wir können nach einer Alternative fragen.<div class="empty-actions"><button class="btn secondary" type="button" data-we="inquiry">Alternative anfragen</button></div></div>';return}box.innerHTML=results.map(h=>{const m=hostMeta(h);return `<article class="host-card host-card-rich"><div class="host-card-top"><span class="eyebrow">${online?'LIVE FREI':'PARTNER'}</span>${m?.accommodation_type?`<small>${esc(m.accommodation_type)}</small>`:''}</div><h3>${esc(h.name)}</h3><div class="location">${esc(h.location||'')}</div><div class="host-price">${h.price!=null?`<strong>${money(h.price)}</strong><span>für diese Anfrage</span>`:'<strong>Preis auf Bestätigung</strong><span>direkt vom Gastgeber</span>'}</div><div class="chips"><span class="chip">1 Nacht</span><span class="chip">Direktzahlung</span>${featureChips(h)}</div><div class="host-actions">${m?.direct_url?`<a class="btn secondary" href="${esc(m.direct_url)}" target="_blank" rel="noopener">Gastgeber ansehen</a>`:''}<button class="btn primary" type="button" data-book='${encodeURIComponent(JSON.stringify({hostId:h.hostId||h.id,name:h.name,location:h.location,date,guests,price:h.price??null}))}'>Gastgeber auswählen</button></div></article>`}).join('');box.querySelectorAll('[data-book]').forEach(btn=>btn.addEventListener('click',()=>openBooking(JSON.parse(decodeURIComponent(btn.dataset.book)))))}
...
async function submitBooking(e){
  e.preventDefault();track('trip_request_started',{route:$('route').value,nights:activeBooking?.items?.length||0,luggage:!!currentTrip?.luggage});if(!activeBooking){setBookingState('Bitte zuerst Gastgeber auswählen.',true);return}
  const submit=e.submitter||$('bookingForm').querySelector('button[type="submit"]'),oldText=submit.textContent,refs=[];const tripKey=activeBooking.tripKey||(activeBooking.tripKey=((globalThis.crypto&&crypto.randomUUID)?crypto.randomUUID():`${Date.now()}-${Math.random().toString(36).slice(2)}-${Math.random().toString(36).slice(2)}`));let tripReference='';submit.disabled=true;submit.textContent='Wird gesendet …';setBookingState(`Anfrage an ${activeBooking.items.length} Gastgeber wird übertragen …`);
  try{
    for(const item of activeBooking.items){const payload={hostId:item.hostId,stayDate:item.date,guests:Number($('bookingGuests').value||1),guestName:$('guestName').value.trim(),guestEmail:$('guestEmail').value.trim(),guestPhone:$('guestPhone').value.trim(),note:$('bookingNote').value.trim(),price:item.price??null,paymentMethod:'host',tripKey,clientRequestId:`${tripKey}:${item.hostId}:${item.date}`};const r=await fetch(`${API_BASE}/api/guest-bookings`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const result=await r.json().catch(()=>({}));if(!r.ok)throw new Error(result.error||'Anfrage konnte nicht gesendet werden.');refs.push(result.reference);tripReference=result.tripReference||tripReference}
    saveTripDashboard(refs,tripKey,tripReference);track('trip_request_submitted',{route:$('route').value,nights:refs.length,luggage:!!currentTrip?.luggage});setBookingState(`✓ Reiseanfrage ${tripReference||''} übertragen. ${refs.length} Übernachtung${refs.length===1?'':'en'} warten auf Bestätigung. Den Gesamtstatus siehst du unter „Meine Reise“ auf diesem Gerät.`);submit.textContent='Anfrage gesendet ✓';
  }catch(err){if(refs.length){saveTripDashboard(refs,tripKey,tripReference);setBookingState(`${refs.length} Übernachtung${refs.length===1?' wurde':'en wurden'} bereits übertragen. Der Rest konnte nicht gesendet werden: ${err.message}. Bitte erneut versuchen; bereits gesendete Nächte werden nicht doppelt angelegt.`,true)}else setBookingState(`Anfrage konnte nicht gesendet werden: ${err.message}`,true);submit.disabled=false;submit.textContent=oldText}
}
...
track('planner_view',{route:$('route').value});closeBooking();const qs=new URLSearchParams(location.search),tomorrow=new Date();tomorrow.setDate(tomorrow.getDate()+1);const todayIso=iso(new Date());$('startDate').min=todayIso;$('stayDate').min=todayIso;$('startDate').value=qs.get('startDate')||iso(tomorrow);$('stayDate').value=qs.get('startDate')||iso(tomorrow);if(qs.get('dailyKm'))$('dailyKm').value=qs.get('dailyKm');if(qs.get('guests')){$('guests').value=qs.get('guests');$('stayGuests').value=qs.get('guests')}if(qs.get('luggage')==='1'){$('luggage').checked=true;$('stayLuggage').checked=true}loadData().catch(err=>{console.error(err);alert('WachauEtappe konnte die Routendaten nicht laden.')});