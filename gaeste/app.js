...
async function submitBooking(e){
  ...
  const submit=e.submitter||$('bookingForm').querySelector('button[type="submit"]'),oldText=submit.textContent,refs=[];const tripKey=(globalThis.crypto&&crypto.randomUUID)?crypto.randomUUID():`${Date.now()}-${Math.random().toString(36).slice(2)}-${Math.random().toString(36).slice(2)}`;let tripReference='';submit.disabled=true;submit.textContent='Wird gesendet …';setBookingState(`Anfrage an ${activeBooking.items.length} Gastgeber wird übertragen …`);
  try{
