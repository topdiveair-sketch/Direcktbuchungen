...
export async function executeApprovedAction(execution, approval, env = {}) {
  ...
  const message = String(approval.note || '').trim();
  ...
  try {
    ...
    return { ...execution, status: 'executed', attempts: Number(execution.attempts || 0) + 1, executedAt: now(), updatedAt: now(), result: detail };
  } catch (error) {
