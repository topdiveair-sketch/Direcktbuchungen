...
export function buildExecution(approval = {}) {
  ...
  return { id: crypto.randomUUID(), approvalId: approval.id, brand: approval.brand, kind: approval.kind, task: approval.task, channel: approval.channel || null, status: 'queued', createdAt: now(), attempts: 0 };
}
...
export async function executeApprovedAction(execution, approval, env = {}) {
  ...
  const response = await fetch(webhook, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(env.CHANNEL_WEBHOOK_TOKEN ? { Authorization: `Bearer ${env.CHANNEL_WEBHOOK_TOKEN}` } : {}) }, body: JSON.stringify({ approvalId: approval.id, brand: approval.brand, kind: approval.kind, task: approval.task, channel: approval.channel || null, rationale: approval.rationale || null, approvedAt: approval.decidedAt || null }) });
  if (!response.ok) return { ...execution, status: 'failed', attempts: Number(execution.attempts || 0) + 1, lastHttpStatus: response.status, updatedAt: now() };
  return { ...execution, status: 'executed', attempts: Number(execution.attempts || 0) + 1, executedAt: now(), updatedAt: now() };
}
